...
const Login: React.FC = () => {
    ...
    // Get prompt from location state if any
    ...
    const onFinish = (values: any) => {
        ...
        setTimeout(() => {
            setLoading(false);
            if (values.email === 'alican@tradevision.com' && values.password === 'trade123') {
                login('Admin');
                message.success('Giriş başarılı! Yönetici olarak yönlendiriliyorsunuz.');
                navigate(from, { replace: true });
            } else if (values.email === 'user@tradevision.com' && values.password === 'user123') {
                login('User');
                message.success('Giriş başarılı! Kullanıcı olarak yönlendiriliyorsunuz.');
                ...
                <div className="w-1/2 bg-black text-white flex flex-col justify-between p-12 relative overflow-hidden">
                    <div className="z-10">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="px-3 py-1 bg-white text-black text-xl font-bold rounded">TV</div>
                            <span className="text-2xl tracking-wide font-light">Trade<span className="font-bold">Vision</span></span>
                        </div>
                        <Title level={1} style={{ color: 'white', fontWeight: 700, margin: 0 }}>
