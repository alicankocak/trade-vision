...
import { intacAlerts, riskData, trendData } from '../utils/mockData'
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  if (active && payload && payload.length) {
    return (
      <div
        className={`p-3 rounded border shadow-sm ${isDarkMode ? 'bg-[#1a1a1a] text-white border-[#2d2d2d]' : 'bg-white text-black border-[#e2e2e4]'}`}
      >
        <p
          className={`font-semibold mb-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
        >
          {label}
        </p>
...
const Dashboard: React.FC = () => {
  ...
  const dashboardKPIs = [
    ...
    },
    {
      ...
      icon: (
        <SafetyCertificateOutlined
          className={`text-xl ${isDarkMode ? 'text-white' : 'text-[#000000]'}`}
        />
      ),
  ...
  const dynamicRiskData = riskData.map((item) => ({
    ...
    color:
      item.name === 'Yüksek'
        ...
        : isDarkMode
          ? '#525252'
          : item.color, // Adjust grays for dark mode
  }))
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div
        className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-4 rounded-lg border shadow-sm ${isDarkMode ? 'bg-[#1a1a1a] border-[#2d2d2d]' : 'bg-white border-[#e2e2e4]'}`}
      >
        <div>
          <Title
            level={2}
          ...
          <Text
            type="secondary"
            style={{ color: isDarkMode ? '#a3a3a3' : undefined }}
          >
            İthalat ve ihracat operasyonlarınızın genel görünümü.
          </Text>
        </div>
        <Space wrap>
            ...
            >
              <div className="flex flex-col justify-between h-full gap-2">
                <div className="flex justify-between items-start">
                  ...
                  {kpi.suffix && (
                    <Tag
                      className={`m-0 text-[10px] uppercase font-bold border-none ${isDarkMode ? 'bg-[#262626] text-gray-400' : 'bg-gray-100 text-gray-500'}`}
                    >
                      {kpi.suffix}
                    </Tag>
                  )}
                ...
                <div>
                  <Statistic
                    ...
                    valueStyle={{
                      fontWeight: 700,
                      fontSize: '24px',
                      color: kpi.color || (isDarkMode ? 'white' : 'black'),
                    }}
                  />
                  <p
                    className={`text-xs font-medium uppercase tracking-wide mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                  >
                    {kpi.title}
                  </p>
                </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                  ...
                  <XAxis
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={{ fontSize: 12, fill: secondaryColor }}
                  />
                  <YAxis
                    axisLine={false}
                    tickLine={false}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} md={12}>
          <Card
            ...
            extra={
              <a
                className={`text-xs ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-black'}`}
              ...
              renderItem={(item) => (
                ...
                >
                  <List.Item.Meta
                    title={
                      <span
                        className={`font-medium text-sm ${isDarkMode ? 'text-gray-200' : 'text-black'}`}
                      >
                        {item.id}
                      </span>
                    }
                  ...
                  <Tag
                    color="orange"
                    className={`mr-0 border-none ${isDarkMode ? 'bg-[#431407] text-orange-400' : 'bg-orange-50 text-orange-600'}`}
                  >
                    {item.date}
                  </Tag>
                </List.Item>
