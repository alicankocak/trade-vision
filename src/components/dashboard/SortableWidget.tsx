...
export const SortableWidget: React.FC<SortableWidgetProps> = ({ id, children, isEditing, colSpan = 3, rowSpan = 3, onResize, onResizeHeight }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className="h-full group">
            {isEditing && (
                <div className="absolute inset-0 z-50 pointer-events-none border-2 border-dashed border-gray-300 rounded-[8px] bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-start items-end p-2 gap-2">
                    ...
                    {(onResize || onResizeHeight) && (
                        <div className="pointer-events-auto bg-white shadow-sm rounded-lg p-3 flex flex-col gap-3 w-40">
                            {onResize && (
